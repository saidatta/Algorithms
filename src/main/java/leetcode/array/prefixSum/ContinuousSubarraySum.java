...
//
//    Imagine we're computing the prefix sum for every index in our nums array. Let's call these sums sum[0], sum[1], ..., sum[i], ..., sum[j], ... for indices 0, 1, ..., i, ..., j, ... respectively.
//
...
//
//            Why?
//    Because the sum of numbers between index i+1 and j is essentially sum[j] - sum[i]. If this number modulo k is 0, then it's a multiple of k.
//
...
//    sum[4] = (2+3+1+2+2) % 5 = 0
//            ...
//    Now, notice that sum[1] and sum[4] both have a modulo of 0 with respect to 5. This means the sum of numbers between indices 2 and 4 (1-indexed) or indices 1 and 4 (0-indexed) is a multiple of 5.
//
//    In this case, that subarray is [1, 2, 2], which indeed sums to 5, a multiple of k.
//
//    This is how the prefix sum and the modulo property together help determine if a subarray exists whose sum is a multiple of k.
